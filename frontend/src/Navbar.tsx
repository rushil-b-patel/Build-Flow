...
export default function Navbar({ user, onLogout }: NavbarProps) {
  ...
  return (
    <nav className="w-full max-w-6xl mx-auto px-6 md:px-10 py-4 flex items-center justify-between gap-4">
      <Link to="/" className="text-xl font-bold tracking-tight text-slate-900 hover:text-slate-700 transition-colors flex items-center gap-2">
      ...
      >
        <span className={`block w-5 h-0.5 bg-slate-600 transition-all ${menuOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
            ...
            {user ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img src={user.avatar_url} alt={user.login} className="w-8 h-8 rounded-full ring-2 ring-slate-200" />
