...
function statusConfig(status: string) {
  switch (status) {
    case 'deployed':
      return { icon: CheckCircle2, label: 'Deployed', color: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' };
    case 'error':
      return { icon: XCircle, label: 'Failed', color: 'text-rose-500', bg: 'bg-rose-500/10', border: 'border-rose-500/20' };
    case 'building':
      return { icon: Loader2, label: 'Building', color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20', spin: true };
    case 'queued':
      return { icon: Clock, label: 'Queued', color: 'text-sky-500', bg: 'bg-sky-500/10', border: 'border-sky-500/20' };
    case 'cloning':
      return { icon: GitBranch, label: 'Cloning', color: 'text-violet-500', bg: 'bg-violet-500/10', border: 'border-violet-500/20', spin: true };
    case 'uploading':
      return { icon: Loader2, label: 'Uploading', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', spin: true };
    default:
      return { icon: Clock, label: status, color: 'text-slate-500', bg: 'bg-slate-500/10', border: 'border-slate-500/20' };
  }
}
    ...
    <div className="w-full max-w-6xl mx-auto p-6 md:p-10">
      ...
      {deployments.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 p-12 text-center">
          ...
          <p className="text-slate-500 font-medium mb-1">No deployments yet</p>
        ...
        <div className="space-y-3">
          {deployments.map((d) => {
            ...
            const Icon = cfg.icon;
            const projectUrl = `http://${d.id}.${DEPLOY_URL}/index.html`;
              ...
              >
                <div className="flex flex-col gap-3 sm:hidden">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-400">{relativeTime(d.created_at)}</span>
                  </div>
                  <a
                    href={d.repo_url}
                    target="_blank"
                  ...
                  {d.status === 'deployed' && (
                    ...
                    >
                      {projectUrl} <ArrowUpRight size={12} />
                    </a>
                  )}
                  {d.error && (
                    <p className="text-xs text-rose-500 truncate">{d.error}</p>
                  )}
                </div>
                ...
                <div className="hidden sm:flex items-center gap-4">
                  ...
                  <div className="ml-auto flex items-center gap-4 shrink-0">
                    ...
                    {d.error && (
                      <span className="text-xs text-rose-500 truncate max-w-[160px]" title={d.error}>
                        {d.error}
                      </span>
                    )}
